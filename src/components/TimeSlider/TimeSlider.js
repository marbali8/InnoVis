...
const TimeSlider = ({ height = 300, width = 500 }) => {
    ...
    useEffect(() => {
        ...
        // append svg
        var svg_ = d3.select(svgRef.current)
                     .attr('width', width)
                     .attr('height', height);
    ...
    }, [height, margin.left, margin.right, range, step, width]);

    // style property is passed an object with height and width converted to strings using template literals
    return <div style={{ height: `${height}`, width: `${width}` }} className={classes.timeSlider}></div>;
};
