...
and in megaballs we use d3 to select the elements mounted here and change them
  ...
  **/
  useEffect(() => {
    ...
    if (!didMount.current) {
      const companyInfoBox = d3.select(anchor.current).append('div').classed('companyInfoBoxContainer', true).attr('opacity', 0);

      const card = companyInfoBox.append('div').classed('companyInfoBox_companyName', true);

      card.append('h1').classed('companyInfoBox_companyName', true);
      card.append('p').classed('companyInfoBox_founders', true);
      card.append('p').classed('companyInfoBox_year');
      card.append('a').classed('companyInfoBox_website');
    }
  }, []);

  return <svg ref={anchor} className={"CompanyInfoBox"} height={width} width={height} />
}
